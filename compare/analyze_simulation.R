...
if (param == "RR") {
  ...
  result <- cbind(
    result.brm, result.CMH, result.LB, result.LP,
  ...
  colnames(result) <- c(
    "brm", "CMH", "LB", "LP", "RLP", "brm-FC", "brm-BC",
  ...
  result <- cbind(
    result.brm, result.bayesian, result.GLM, result.LPM,
...
if (param == "RR") {
  data_names <- c(
    "data.brm", "data.CMH", "data.lb", "data.lp",
  ...
  data_names <- c(
    "data.brm", "data.brm.ad", "data.bayes", "data.glm",
  ...
  est <- do.call(cbind, lapply(data_list, `[[`, "estimate"))
  colnames(est) <- c(
    "brm", "brm_b", "bayesian", "GLM", "LPM", "MN", "brm-FC", "brm-BC",
...
p1 <- ggplot(est_long_small, aes(x = method, y = estimate, fill = method)) +
  ...
  labs(
    title = paste0("Monte Carlo when n = ", n),
    x = "Method", y = "Estimate"
  )
...
p3 <- ggplot(se_long_small, aes(x = method, y = SE, fill = method)) +
  ...
  labs(
    title = paste0("Boxplot of RR for n = ", n),
    x = "Method", y = "SE"
  )
...
p_acc <- plot_metric(
  df, "acc", paste0("Barplot of accuarcy at n = ", n), 1, 1.5,
  paste0("accuracy ", param, " ", event, " ", hypothesis, " n = ", n, ".png")
)
p_cov <- plot_metric(
  df, "coverage", paste0("Barplot of coverage at n = ", n), 0.95, 1.1,
