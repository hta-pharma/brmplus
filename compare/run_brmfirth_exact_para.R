src_all <- function(){
  ...
  source("bayes_p.R")
  source("MyFunc.R")
  source("CI_exact_adapt_para_noearlystop.R")
...
simulate.rr <- function(n, event, hypothesis, seed,
  ...
  ###result
  point.est <- as.vector(c(est.brm$point.est[1],          # brm
  ...
  colnames(result.comp) <- c("brm", "brm_firth", "brm_firth_exact",
...
simulate.rd <- function(n, event, hypothesis, seed,
  ...
  ##brm
  est.brm <- MLEst('RD', y, x, va, vb, weight, max.step, thres, alpha.start = rep(0, pa),
                   beta.start = rep(0, pb), pa, pb)
...
run_scenario <- function(param, n, event, hypothesis, R,
  ...
  if (ncores > 1L) {
    ...
    clusterEvalQ(cl, {
      suppressPackageStartupMessages({
        library(brmplus);library(epitools);library(geepack);library(sandwich)
  ...
  if (ncores == 1L) {
    res <- foreach(r = seed_vec) %do% {
      one_rep(r, param, n, event, hypothesis, max.step = max.step,
  ...
  # --- ��ÿ�� replicate �� full (5 x K) �ѵ��� full_arr: 5 x K x Rrun ---
