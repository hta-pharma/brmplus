...
m <- function(x){exp(x)/(1+exp(x))}
m.prime <- function(x){exp(x)/(1+exp(x))^2}
m.prime.prime <- function(x){exp(x)*(1-exp(x))/(1+exp(x))^3}

fish <- function(x,beta){t(x)%*%diag(as.vector(m.prime(x%*%beta)))%*%x/nrow(x)}
