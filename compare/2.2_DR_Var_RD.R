#' Sandwich Variance Estimator for Doubly‐Robust RD Estimator
#'
#' Compute the asymptotic variance of the doubly‐robust risk‐difference (RD)
#' 
...
var.rd.dr = function(y, x, va, vb, vc, alpha.dr, alpha.ml, beta.ml, gamma, 
    ...
    Hrd = Hessian2RD(y, x, va, vb, alpha.ml, beta.ml, weight)
    hessian = Hrd$hessian
    ...
    if (optimal == TRUE) {
        ...
        dwt.by.dalpha.beta = matrix(0, n, 1 + ncol(vb))
    }
    
    dU.by.dalpha.ml.beta.ml = t(dU.by.dp0 * weight) %*% (dp0.by.dalpha.beta) + 
        t(dU.by.dwt * weight) %*% dwt.by.dalpha.beta
    ...
    dU.by.dgamma = t(dU.by.dpscore * weight) %*% dpscore.by.dgamma + t(dU.by.dwt * 
        weight) %*% dwt.by.dgamma  # 2 by 2
    ...
    Utilde = U - t(dU.by.dalpha.ml.beta.ml %*% (-ginv(hessian)) %*% t(S)) - 
        t(dU.by.dgamma %*% (ginv(part4)) %*% t(pscore.score))  # n by 2
    ...
    alpha.dr.variance = ginv(tau) %*% USigma %*% ginv(tau)/sum(weight)
