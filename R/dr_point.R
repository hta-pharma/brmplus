...
dr_estimate_noiterate <- function(
  ...
  if (optimal == TRUE) {
    if (param == "RR") {
      p0 <- get_prob_rr(mat_vec_mul(va, alpha.ml), mat_vec_mul(vb, beta.ml))[, 1]
      wt <- as.vector(1 / (1 - p0 + (1 - pscore) * (exp(mat_vec_mul(-va, alpha.ml)) -
        1)))
    }
    if (param == "RD") {
      p0 <- get_prob_rd(mat_vec_mul(va, alpha.ml), mat_vec_mul(vb, beta.ml))[, 1]
  ...
  alpha.dr.opt <- dr_estimate_onestep(
