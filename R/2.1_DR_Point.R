dr.estimate.onestep <- function(
    param, y, x, va, vb, alpha.start, beta, pscore,
  ...
  if (param == "RR") {
    dr.objective <- function(pars) {
      p0 <- getProbRR(mat_vec_mul(va, startpars), mat_vec_mul(vb, beta))[, 1]
      H.alpha <- as.vector(y * exp(-x * (va %*% pars)))
      tmp <- mat_vec_mul(t(va), (wt * (x - pscore) * (H.alpha - p0) * weights))
      return(sum(tmp^2))
    }
  }
  if (param == "RD") {
    dr.objective <- function(pars) {
      p0 <- getProbRD(mat_vec_mul(va, startpars), mat_vec_mul(vb, beta))[, 1]
      H.alpha <- y - x * tanh(mat_vec_mul(va, pars))
      tmp <- mat_vec_mul(t((H.alpha - p0) * (x - pscore)), (va * wt * weights))
      return(sum(tmp^2))
    }
  }
...
dr.estimate.noiterate <- function(
  ...
  if (optimal == TRUE) {
    if (param == "RR") {
      p0 <- getProbRR(mat_vec_mul(va, alpha.ml), mat_vec_mul(vb, beta.ml))[, 1]
      wt <- as.vector(1 / (1 - p0 + (1 - pscore) * (exp(mat_vec_mul(-va, alpha.ml)) -
        1)))
    }
    if (param == "RD") {
      p0 <- getProbRD(mat_vec_mul(va, alpha.ml), mat_vec_mul(vb, beta.ml))[, 1]
      rho <- as.vector(tanh(mat_vec_mul(va, alpha.ml)))
  ...
  alpha.dr.opt <- dr.estimate.onestep(
    param, y, x, va, vb, alpha.start, beta.ml,
